...
void merge(long n, T left[n], T right[n], T result[n*2], long start, long length, unsigned int depth) {
...
void multisort(long n, T data[n], T tmp[n], unsigned int depth) {
    if (n >= MIN_SORT_SIZE*4L) {
        ...
        #pragma omp task final(depth > CUTOFF) mergeable depend(in: data[0], data[n/4L]) depend(out: tmp[0])
        merge(n/4L, &data[0], &data[n/4L], &tmp[0], 0, n/2L, depth+1);
        #pragma omp task final(depth > CUTOFF) mergeable depend(in: data[n/2L], data[3L*n/4L]) depend(out: tmp[n/2L])
        merge(n/4L, &data[n/2L], &data[3L*n/4L], &tmp[n/2L], 0, n/2L, depth+1);

        #pragma omp task final(depth > CUTOFF) mergeable depend(in: tmp[0], tmp[n/2L])
...
void check_sorted(long n, T data[n]) 
{
    ...
    if (unsorted > 0)
        printf ("\nERROR data is NOT properly sorted. Number of unordered positions: %d\n\n", unsorted);
}
